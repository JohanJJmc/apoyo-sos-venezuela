...
export function RequestFormModal({
  ...
  const [requesterName, setRequesterName] = useState("Johan meneses");
  ...
  const canSubmit = Boolean(category && item && quantity > 0);
  ...
  useEffect(() => {
    ...
    setPhotoError("");
    setRequesterName("Johan meneses");
    setRequesterPhone("");
      ...
      >
        ...
        <div className="space-y-5">
          ...
          <section className="space-y-3">
            ...
            <TextInput label="" value={requesterName} onChange={(event) => setRequesterName(event.target.value)} placeholder="Nombre" />
            <TextInput label="" value={requesterPhone} onChange={(event) => setRequesterPhone(event.target.value)} placeholder="📞 Telefono" />
            <label className="flex min-h-12 items-center gap-3 rounded-input border border-sos-border bg-sos-background px-4 text-[16px] font-semibold text-sos-ink">
              <input type="checkbox" checked={requesterAnonymous} onChange={(event) => setRequesterAnonymous(event.target.checked)} className="h-4 w-4" />
              Anonimo
            </label>
          </section>
